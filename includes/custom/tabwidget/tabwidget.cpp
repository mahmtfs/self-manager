...
TabWidget::TabWidget(Widget* parent)
    : Widget(parent)
    , mHeader(new custom::TabHeader(nullptr)) // create using nullptr, add children below
    , mContent(new nanogui::StackedWidget(nullptr)) {
...
nanogui::Widget* TabWidget::createTab(int index, const std::string &label) {
    Widget* tab = new Widget(nullptr);
    addTab(index, label, tab);
    return tab;
}

nanogui::Widget* TabWidget::createTab(const std::string &label) {
...
const nanogui::Widget *TabWidget::tab(const std::string &tabName) const {
...
nanogui::Widget *TabWidget::tab(const std::string &tabName) {
    int index = mHeader->tabIndex(tabName);
    if (index == -1 || index == mContent->childCount())
        return nullptr;
    return mContent->children()[index];
...
const nanogui::Widget *TabWidget::tab(int index) const {
    if (index < 0 || index >= mContent->childCount())
        return nullptr;
    return mContent->children()[index];
}

nanogui::Widget *TabWidget::tab(int index) {
    if (index < 0 || index >= mContent->childCount())
        return nullptr;
    return mContent->children()[index];
}
...
nanogui::Vector2i TabWidget::preferredSize(NVGcontext* ctx) const {
    auto contentSize = mContent->preferredSize(ctx);
    auto headerSize = mHeader->preferredSize(ctx);
    int margin = mTheme->mTabInnerMargin;
    auto borderSize = nanogui::Vector2i(2 * margin, 2 * margin);
    nanogui::Vector2i tabPreferredSize = contentSize + borderSize + nanogui::Vector2i(0, headerSize.y());
